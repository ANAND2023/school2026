...
const MenuSidebar = () => {
  ...
  // --- UPDATED: One menu at a time logic ---
  const handleToggleGroup = (groupName) => {
    if (menuSidebarCollapsed) return;
    setExpandedMenus(prev => {
      // If the clicked group is already open, close it (return empty object).
      // If it's closed, open it and close everything else (return object with only this key).
      const isCurrentlyOpen = !!prev[groupName];
      return isCurrentlyOpen ? {} : { [groupName]: true };
    });
  };
      ...
      >
        ...
        <div className="md-sidebar-content">
          {filteredMenu.map((group, index) => {
             ...
             // --- UPDATED: Check if this is Dashboard to treat as Direct Link ---
             const isDashboard = group.name === "Dashboard";
             ...
             if (isDashboard) {
                ...
                return (
                    <div key={groupKey} className="md-menu-group">
                        <NavLink
                            to="/dashboard"
                            className="md-nav-group-header"
                            onClick={handleCloseMobile}
                            style={{
                                color: isActive ? currentTheme.primary : currentTheme.sidebarText,
                                backgroundColor: isActive ? currentTheme.sidebarGroupHover : 'transparent',
                                cursor: 'pointer',
                                textDecoration: 'none',
             ...
             // --- Standard Menu Group Rendering (Accordion) ---
             return (
               <div key={groupKey} className="md-menu-group">
                 {/* Parent Menu Item */}
                 <div 
        ...
        <div style={{
          ...
          minWidth: '280px',
          display: menuSidebarCollapsed ? 'none' : 'block'
        }}>
