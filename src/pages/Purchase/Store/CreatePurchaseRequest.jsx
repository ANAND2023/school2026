...
const CreatePurchaseRequest = () => {
  ...
  const handleSavePurchaseRequest = async () => {
    ...
    const filteredBodyData = bodyData.slice(0, bodyData.length - 1)?.filter((val)=>val?.ItemName?.ItemID);
    const Quantity = filteredBodyData.some((item) => Number(item.Quantity) > 0);
    // const Quantity = filteredBodyData.some((item) => item.Quantity == "");
    // const Remark = filteredBodyData.some((item) => item.Remark == "");
    const Remark = filteredBodyData.some((item) => item.Remark ? item.Remark.length > 1000 : null);
    let department = payload?.departmentTo?.value;
    let item = bodyData

    if (!department) {
      notify("select department ", "warn");
      return
    }
    if (item.length < 2) {
      notify("Please Select Item First.", "warn");
      return
    }
    if (!payload.requisitionType) {
      notify("Please Select requisitionType.", "warn");
      return
    }
    if (!Quantity) {
      notify("Invalid Quantity ", "warn");
      return
    }
    ...
    if (!payload.narration) {
      notify("Narration is Required", "warn");
      return
    }
    if (payload.narration?.length>1000) {
      notify("Narration should not exceed 1000 characters.", "warn");
      return
    }
    try {
      let data = {
        ...
        item: filteredBodyData?.map((item) => ({
          ...
          indentQuantity: 0,
          salesQuantity: 0,
          netQuantity: Number(item?.Quantity??0),
          indentNumber: "",
          id: 0
        }))
    ...
    try {
      let data = {
        ...
        item: filteredBodyData.map((item) => {
          ...
          const isItemNameObject = typeof item?.ItemName === "object" && item.ItemName !== null;
          const itemDetails = isItemNameObject ? item.ItemName : {};
          const stockDetails = item?.stockDetails || {};

          return {
            itemName: isItemNameObject ? itemDetails.ItemName || "" : item.ItemName || "",
            subCategoryID: isItemNameObject ? itemDetails.SubCategoryID ?? 0 : item.SubCategoryID ?? 0,
            itemID: isItemNameObject ? itemDetails.ItemID ?? 0 : item.ItemID ?? 0,
            minlevel: item.MinLevel ?? stockDetails.MinLevel ?? 0,
            maxlevel: item.MaxLevel ?? stockDetails.MaxLevel ?? 0,
            quantity: Number(item.Quantity ?? 0),
            narration: item.Remark ?? "",
            currentStock: item.CurrentStock ?? stockDetails.StockQuantity ?? 0,
            poStock: item.PoStock ?? stockDetails.POQuantity ?? 0,
            purchaseUnit: isItemNameObject ? itemDetails.MajorUnit ?? null : item.PurchaseUnit ?? null,
