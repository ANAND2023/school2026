...
const GeneralLedger = () => {
    ...
    const handleSearch = async () => {
        ...
        if (!payloadData?.coa_cog_wise?.value) {

            notify("Please select Chart Of CAO/COG  ", "error")
            return
        }
        const payload = {
            "centreID": Number(payloadData?.BranchCentre?.value),
            "currencyCode": String(payloadData?.Currency?.value),
            "voucherType": String(payloadData?.VoucherType?.value),
            "acFilterType": Number(payloadData?.coType?.value),
            "acFilterValue": String(payloadData?.coa_cog_wise?.value),
    ...
    const validateInvestigation = async (e, name) => {
        ...
        if (name === "coa_cog_wise") {
            value.isAccountType = true
            setPayloadData((val) => ({ ...val, ["coa_cog_wise"]: value }))
    ...
    const handleExportPDF = (e) => {
        ...
        exportHtmlToPDFNoPrint("printSection", "General Ledger", "1000px", "landscape")
    };
    const handleExportExcel = async (tableData) => {

        // let apiResp = await GetBankChargesUploadExcel();
        if (tableData) {
            exportToExcel(transformDataInTranslate(tableData, t), "General Ledger Report");
        } else {
        ...
        <>
            <div className="mt-2 spatient_registration_card">
                <div className="patient_registration card">
                    {/* <Heading isBreadcrumb={false} title={t("General Ledger")} /> */}
                    <div className="row p-2">
                        ...
                        <ReactSelect
                            ...
                            // id="coType"
                            name="coType"
                            value={payloadData?.coType?.value}
                            removeIsClearable={true}
                            handleChange={(name, value) => setPayloadData((val) => ({ ...val, [name]: value }))}

                            dynamicOptions={[
                                {
                                    label: "COG Wise", value: "3"
                                },
                                {
                                    label: "COA Wise", value: "4"
                                },
                        ...
                        <div className="col-xl-4 col-md-8 col-sm-6 col-12">
                            ...
                            <AutoComplete
                                value={payloadData?.coa_cog_wise?.label ? payloadData?.coa_cog_wise?.label : payloadData?.coa_cog_wise}
                                //   value={values?.AccountName?.TextField ? values?.AccountName?.TextField : values?.AccountName}
                                suggestions={items}
                                completeMethod={(e) => { search(e, "coa_cog_wise") }}
                                onChange={(e) => setPayloadData({ ...payloadData, coa_cog_wise: e.value })}
                                className="w-100"
                                onSelect={(e) => validateInvestigation(e, "coa_cog_wise")}
                                id="coa_cog_wise"
                                itemTemplate={itemTemplate}
                            //   onBlur={() => { setPayloadData((prev) => ({ ...prev, coa_cog_wise: "", coa_cog_wise: "" })) }}
