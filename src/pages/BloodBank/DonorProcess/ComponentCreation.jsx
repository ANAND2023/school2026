...
import { BloodBankComponentSearch } from "../../../networkServices/BloodBank/BloodBank";
...
function ComponentCreation() {
  ...
  const handleSearchData = async () => {
    const payload = {
      ...
      bagNo: values?.tubeNo,
      bagType: values?.bagType?.value ?? "All",
      collectFrom: moment(values?.fromDate).format("YYYY-MM-DD"),
      collectTo: moment(values?.toDate).format("YYYY-MM-DD"),
    };
    try {
      const response = await BloodBankComponentSearch(payload);
      if (response.success) {
  ...
  return (
    <>
      ...
      <div className="card">
        <Heading title={t("Blood Screening")} isBreadcrumb={true} />
