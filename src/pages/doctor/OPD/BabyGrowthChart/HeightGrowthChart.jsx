...
const HeightGrowthChart = ({ apiResponse, Sex }) => {
  // Process the API response to prepare chart data
  const labels = apiResponse.map((item) => item.age); // X-axis (age)
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Baby ${Sex === "Male" ? "Boy" : "Girl"} Height Growth Chart`,
      },
    },
    scales: {
      x: {
      ...
      y: {
        title: {
          display: true,
          text: "Height (cm)", // Updated for height
        },
        ticks: {
...
export default HeightGrowthChart;
