...
import { BillingMisCellaneousReport } from '../../../../networkServices/BillingsApi';
...
const MiscellaneousReport = () => {
    ...
    const handleReport = async () => {
        ...
        try {
            const response = await BillingMisCellaneousReport(payload);
            if (response?.success) {
                if (values?.fileType == 0) {


                    exportToExcel(response?.data, ` Miscellaneous Report ${moment(values?.fromDate).format("YYYY-MM-DD")} to ${moment(values?.toDate).format("YYYY-MM-DD")} `);
                } else {
    ...
    return (
        <div className="card">
            <Heading isBreadcrumb={false} title={"Miscellaneous Report"} />
