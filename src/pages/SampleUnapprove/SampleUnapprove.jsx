...
import {
  ...
  SearchunprocesssamplelabPatientLab,
  UpdateUnProcessLab,
...
function SampleUnapprove() {
  ...
  useEffect(() => {
    ...
    handleSearchSampleCollection();
  }, [values.PatientTypeTest.value, values.center.value, values.category.value]);

  const unProcess = async (item,out) => {
    debugger
    try {
      let payload = {
        "testId": item?.Test_ID,
        "labType": out? out : item?.Approved === 1 ? 1 : item?.Result_Flag ? 2 : item?.IsSampleCollected === "Y" ?
          3 : item?.IsSampleCollected === "S" ? 4 : 0,
        "type": values?.category?.value,
        "remark": ""
      }

      const response = await UpdateUnProcessLab(payload);
      if (response?.success) {
  ...
  const handleSearchSampleCollection = async () => {
    ...
    try {
      const apiResp = await SearchunprocesssamplelabPatientLab(payload);
      if (apiResp.success) {
          ...
          <Tables
            ...
            tbody={tbodyPatientDetail
              ...
              .map((val, index) => ({
                ...
                action: (
                  <>
                    {values?.category?.value === "3" &&  val?.IsSampleCollected !== "N" &&<button
                    ...
                    >
                      {val?.Approved === 1 ? t("Unapprove") : val?.Result_Flag ? t("Revert Result") : val?.IsSampleCollected === "Y" ?
                        t("Revert Department Receive") : val?.IsSampleCollected === "S" ? t("Revert Sample Collected") : t("N/A")
                      }
                    </button>}
                    {values?.category?.value === "7" && 
                    (val?.Approved === 1 || val?.Result_Flag === true) &&
                    ...
                    {values?.category?.value === "7" && 
                    (val?.Approved === 0 && val?.Result_Flag === false) &&
                    val?.P_IN === 1 &&<>
...
export default SampleUnapprove;