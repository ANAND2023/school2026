...
const StudentRegistration = () => {
  ...
  const removeParentDocument = (parentIndex, docIndex) => {
    ...
    updatedParents[parentIndex].documents = updatedParents[
      parentIndex
    ].documents.filter((_, i) => i !== docIndex);
    setValues((prev) => ({ ...prev, parents: updatedParents }));
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!values?.lastName)
      return notify("Please enter student lastName", "error");
    if (!values?.dateOfBirth)
      return notify("Please enter student date of birth", "error");
    ...
    if (!values?.parents?.filter((parent) => parent.name).length)
      return notify("Please add parent name", "error");
    if (!values?.parents?.filter((parent) => parent.mobile).length)
      return notify("Please add parent Mobile", "error");
    if (
      !values?.studentDocuments?.filter((student) => student.documentNumber)
        .length
    )
      return notify("Please add document Number", "error");
    try {
      const payload = {
            ...
            <div className="row g-3">
              {Object.keys(values.address).map((key) => (
                <Input
                  ...
                  onChange={(e) =>
                    handleNestedChange("address", key, e.target.value)
                  }
                  respclass="col-xl-2 col-md-4 col-sm-4 col-12"
                />
              ))}
            ...
            <div className="row g-3">
              {Object.keys(values.otherInfo).map((key) => (
                <Input
                  ...
                  onChange={(e) =>
                    handleNestedChange("otherInfo", key, e.target.value)
                  }
                  respclass="col-xl-2 col-md-4 col-sm-4 col-12"
                />
              ))}
              ...
              >
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  {values.parents.length > 1 && (
                    <i
                      className="bi bi-trash me-1 text-danger"
                    ...
                    >
                      <div className="row g-2 align-items-end">
                        ...
                        <ReactSelect
                          ...
                          value={
                            values.parents[parentIndex].documents[docIndex]
                              .documentType
                              ? {
                                  label:
                                    values.parents[parentIndex].documents[
                                      docIndex
                                    ].documentType === "1"
                                      ? "PAN"
                                      : "Aadhaar",
                        ...
                        <div className="col-md-1">
                          <i
                            className="bi bi-trash me-1 text-danger pointer"
                            onClick={() =>
                              removeParentDocument(parentIndex, docIndex)
                            }
                            style={{ cursor: "pointer" }}
                          ></i>
                        </div>
        ...
        <div className="card shadow-sm mb-4">
          <div className="card-header ">
            <Heading
              ...
              isBreadcrumb={false}
              secondTitle={
                <button className="btn  btn-sm" onClick={addStudentDocument}>
                  Add Document
                </button>
              }
            />
          ...
          <div className="card-body">
            <div className="row g-3">
              <Input
                ...
                onChange={(e) =>
                  handleNestedChange("otherDetail", "route", e.target.value)
                }
                respclass="col-md-6"
              />
              <Input
                ...
                onChange={(e) =>
                  handleNestedChange("otherDetail", "busNo", e.target.value)
                }
                respclass="col-md-6"
              />
            </div>
