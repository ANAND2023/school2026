...
import ViewLabReportTable from "../UI/customTable/doctorTable/ViewConsultationtable/ViewLabReportTable";
...
const ViewLabReport = forwardRef((props, ref) => {
  ...
  const handlePacImage = () => {
    window.open(
      `https://mohpacs.actoneng.com:8082/?patient=${PID}`
    );
  };
  ...
  return (
    <>
      <div className="">
        ...
        >
          <ViewLabReportTable
            tbody={Array.isArray(apiData?.getSearchPatient) ? apiData.getSearchPatient : []}
          />
        </div>
      </div>
    </>
